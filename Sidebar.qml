...
PanelWindow {
    ...
    readonly property color stripBg:  "#DDD5A8"
    ...
    Timer {
        ...
        onTriggered: {
            ...
            h = h % 12
            if (h === 0) h = 12
            sidebar.currentTime = h + ":" + (m < 10 ? "0" + m : m) + " " + ampm
        }
    }
    ...
    Process {
        id: weatherProc
        command: ["sh", "-c", "curl -sf 'wttr.in/?format=%t+%C' 2>/dev/null || echo 'no network'"]
        running: true
    ...
    Process {
        id: wifiStrengthProc
        command: ["sh", "-c", "awk 'NR==3{gsub(/\\./, \"\"); print int($3 * 100 / 70)}' /proc/net/wireless 2>/dev/null || echo 0"]
        running: true
    ...
    Process {
        id: volProc
        command: ["sh", "-c", "wpctl get-volume @DEFAULT_AUDIO_SINK@ 2>/dev/null || echo 'Volume: 0.65'"]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                var t = text.trim()
    ...
    function saveTodos() {
        var p = Qt.createQmlObject('import Quickshell.Io; Process {}', sidebar)
        p.command = ["sh", "-c", "echo '" + JSON.stringify(todos).replace(/'/g, "") + "' > ~/.config/quickshell/todos.json"]
        p.running = true
    }

    Component.onCompleted: {
        var p = Qt.createQmlObject('import Quickshell.Io; Process {}', sidebar)
        p.command = ["sh", "-c", "cat ~/.config/quickshell/todos.json 2>/dev/null || echo []"]
        var col = Qt.createQmlObject('import Quickshell.Io; StdioCollector {}', p)
        p.stdout = col
        col.onStreamFinished.connect(function() {
            try { sidebar.todos = JSON.parse(col.text) } catch(e) { sidebar.todos = [] }
        })
        p.running = true
    }
    ...
    Rectangle {
        ...
        width: 6
        color: sidebar.stripBg
        opacity: sidebar.hovered ? 0 : 1
    ...
    Rectangle {
        ...
        Flickable {
            ...
            Column {
                ...
                // CLOCK
                Item {
                    width: parent.width; height: clockCol.implicitHeight + 24
                    Column {
                        id: clockCol
                        anchors { left: parent.left; right: parent.right; margins: 12 }
                        anchors.verticalCenter: parent.verticalCenter
                        spacing: 4
                        Text { text: sidebar.currentTime; font.family: "Serif"; font.pixelSize: 28; font.weight: Font.Black; color: sidebar.fg }
                        Text { text: Qt.formatDate(new Date(), "dddd, MMMM d"); font.family: "Monospace"; font.pixelSize: 9; color: sidebar.mid }
                    }
                }
                ...
                // POWER
                Item {
                    width: parent.width; height: pwCol.implicitHeight + 24
                    Column {
                        id: pwCol
                        anchors { left: parent.left; right: parent.right; margins: 12 }
                        anchors.verticalCenter: parent.verticalCenter
                        spacing: 8
                        Text { text: "POWER"; font.family: "Monospace"; font.pixelSize: 8; font.letterSpacing: 2; color: sidebar.mid }
                        Rectangle {
                            width: parent.width; height: 36; radius: 6; color: sidebar.red
                            visible: sidebar.powerConfirm
                            Row {
                                anchors.centerIn: parent; spacing: 12
                                Text { text: sidebar.powerAction + "?"; font.family: "Monospace"; font.pixelSize: 11; color: sidebar.bg; anchors.verticalCenter: parent.verticalCenter }
                                Rectangle {
                                ...
                                Rectangle {
                                    width: 40; height: 24; radius: 4; color: "transparent"; border.color: sidebar.bg; border.width: 1
                                    Text { anchors.centerIn: parent; text: "no"; font.family: "Monospace"; font.pixelSize: 10; color: sidebar.bg }
                                    MouseArea { anchors.fill: parent; cursorShape: Qt.PointingHandCursor; onClicked: sidebar.powerConfirm = false }
                                }
                            }
                        }
                        Grid {
                    ...
                    Column {
                        ...
                        Row {
                            ...
                            Rectangle {
                                ...
                                anchors.verticalCenter: parent.verticalCenter
                                Text { anchors.centerIn: parent; text: sidebar.volMuted ? "x" : "v"; font.pixelSize: 11; color: sidebar.volMuted ? sidebar.bg : sidebar.fg }
                            ...
                            Item {
                                ...
                                Rectangle {
                                    ...
                                    Rectangle { width: parent.width * (sidebar.volMuted ? 0 : sidebar.volLevel); height: 3; color: sidebar.fg; radius: 2 }
                                }
                                Rectangle {
                        ...
                        Row {
                            ...
                            Row {
                                ...
                                Repeater {
                                    model: 4
                                    Rectangle {
                                        width: 5; height: 6 + index * 4; anchors.bottom: parent.bottom; radius: 1
                ...
                Item {
                    ...
                    Column {
                        ...
                        Row {
                            spacing: 10
                            Text { text: sidebar.batPct + "%"; font.family: "Serif"; font.pixelSize: 22; font.weight: Font.Black; color: sidebar.batPct < 20 ? sidebar.red : sidebar.fg; anchors.verticalCenter: parent.verticalCenter }
                            Text { text: sidebar.batCharging ? "charging" : "on battery"; font.family: "Monospace"; font.pixelSize: 9; color: sidebar.batCharging ? "#689D6A" : sidebar.mid; anchors.verticalCenter: parent.verticalCenter }
                        }
                        Item {
                        ...
                        Repeater {
                            ...
                            Row {
                                ...
                                Rectangle {
                                    ...
                                    MouseArea {
                                        anchors.fill: parent; cursorShape: Qt.PointingHandCursor
                                        onClicked: { var t = sidebar.todos.slice(); t.splice(index, 1); sidebar.todos = t; sidebar.saveTodos() }
                                    }
                                }
                                Text { text: modelData; font.family: "Monospace"; font.pixelSize: 10; color: sidebar.fg; width: todoCol.width - 22; elide: Text.ElideRight; anchors.verticalCenter: parent.verticalCenter }
                            }
                        }
                        Row {
                            width: parent.width; spacing: 6
                            Rectangle {
                                width: parent.width - 36; height: 26; color: "transparent"; border.color: sidebar.line; border.width: 1; radius: 4
                                TextInput { id: todoInput; anchors.fill: parent; anchors.margins: 6; font.family: "Monospace"; font.pixelSize: 10; color: sidebar.fg; clip: true; onAccepted: sidebar.addTodo() }
                            }
                            Rectangle {
                ...
                Item {
                    ...
                    Column {
                        ...
                        Text { anchors.horizontalCenter: parent.horizontalCenter; text: sidebar.pomodoroBreak ? "break" : "focus"; font.family: "Monospace"; font.pixelSize: 9; font.letterSpacing: 2; color: sidebar.mid }
                        Row {
